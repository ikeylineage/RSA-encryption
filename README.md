...
A lightweight, CLI-based asymmetric file encryption tool implemented in C using OpenSSL's libcrypto.
This tool Demonstrates low-level modular arithmetic, binary file I/O, custom key serialization, and multi-session key persistence.
...
[ Input Stream (in) ] ---> [ Read Bytes ] ---> [ BIGNUM Conversion ] ---> [ BN_mod_exp ] ---> [ Write Bytes ] ---> [ Output Stream (out) ]         
